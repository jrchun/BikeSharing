...
-> RMSE_cas : 37.2382, RMSE_reg : 137.6699  
...
-> RMSE_cas : 59.4892, RMSE_reg : 210.4713  
...
-> RMSE_cas : 33.4605, RMSE_reg : 122.6731  
...
**앙상블**
```{r}
N_var <- c('y', 'casual', 'registered', 'season', 'year','month',  'temp', 'atemp') # 다중공선성을 고려한 변수 제거 
X_var <- setdiff(feature, N_var)

#T_model_reg_2011 <- T_model_reg[which(T_model_reg$year == 2011), ]
#T_model_reg_2012 <- T_model_reg[which(T_model_reg$year == 2012), ]
#V_model_reg_2011 <- V_model_reg[which(V_model_reg$year == 2011), ]
#V_model_reg_2012 <- V_model_reg[which(V_model_reg$year == 2012), ]

#fit_zi_reg_2011 <- zeroinfl(formula = form_zi_reg,
#                            data = T_model_reg_2011)

#fit_pr_reg_2012 <- train(form_pr_reg, method = 'glm',  family = 'poisson', data = T_model_reg_2012)
#sqrt(mean((predict(fit_zi_reg_2011, newdata = V_model_reg_2011) - V_model_reg_2011$registered)^2))
#sqrt(mean((predict(fit_pr_reg_2012, newdata = V_model_reg_2012) - V_model_reg_2012$registered)^2))
#V6 <- c(predict(fit_zi_reg_2011, newdata = V_model_reg_2011), predict(fit_pr_reg_2012, newdata = V_model_reg_2012))
#V6 <- V6[index(V6)]
...
ens_data_cas <- as.data.frame(cbind(pred_lm_cas, pred_rr_cas, pred_pr_cas, pred_zi_cas, V_model_cas$casual, V6))
ens_data_reg <- as.data.frame(cbind(pred_lm_reg, pred_rr_reg, pred_pr_reg, pred_zi_reg, V_model_reg$registered, V6))
...
ggplot(data = ens_data_reg, aes(x = index(ens_data_reg), y = pred_pr_reg)) +
  geom_point() +
  geom_point(aes(x = index(ens_data_reg), y = V5, color = 'red')) #크고 작은 데이터들을 전혀 예측하지 못하네

ggplot(data = ens_data_cas, aes(x = index(ens_data_cas), y = pred_pr_cas)) +
  geom_point() +
  geom_point(aes(x = index(ens_data_cas), y = V5, color = 'red')) #얼추 예측이 잘 되는 듯
```
