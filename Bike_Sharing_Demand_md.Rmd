...
#m1 <- glm.nb(formula = form_pr_cas,
#                     data = T_model_cas)

#f1 <- glm(formula = form_pr_cas,
#          family = "quasipoisson",
#          data = T_model_cas)
```
...
```{r}
pred_cas <- predict(f1, newdata = V_model_cas)
...
ggplot(data = ens_data_reg, aes(x = index(ens_data_reg))) +
  geom_point(y = ens_data_reg$pred_lm_reg, color= 'red') +
  geom_point(y = ens_data_reg$pred_rr_reg, color= 'blue', alpha = 0.5) +
  geom_point(y = ens_data_reg$pred_pr_reg, color= 'black', alpha = 0.5) +
  geom_point(y = ens_data_reg$pred_zi_reg, color= 'green', alpha = 0.5) +
  geom_line(y = V_model_reg$registered, alpha = 0.5) +
  ylim(0, max(ens_data_reg[, 1:4]))
...
-> 어려운 모형 < Ridge regression의 아웃풋을 얻을 수 있다.
