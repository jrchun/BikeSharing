...
```{r}
variable_for_windspeed <- c('season', 'weather', 'temp', 'atemp', 'humidity', 'time', 'day', 'month', 'discomfort', 'windspeed')
data_wind <- data[, variable_for_windspeed]

# train_wind / test_wind / val_wind 데이터 분할
train_wind <- data_wind[which(data_wind$windspeed != 0), ]
test_wind <- data_wind[which(data_wind$windspeed == 0), ]
set.seed(1)
val_idx <- createDataPartition(train_wind$windspeed, p = 0.3, list = FALSE)
val_wind <- train_wind[val_idx, ]
train_wind <- train_wind[-val_idx, ]
```

모형 적합전, train에 들어갈 formula와 trainControl인자 설정  
```{r}
...
```{r}
wind_model_tree <- train(f_1, method = 'ctree2',  trControl = tc_1, data = train_wind) #ctree2는 maxdepth도 조절
...
```{r}
#학습시간 쒯~~~~ 해결방법은?
wind_model_rf <- train(f_1, method = 'ranger', data = train_wind) # rf(randomForest보다 빠르대!) trControl = tc_1,
...
**method2 . 중앙값(Median)으로 대치 **  

```{r}
wind_pre_median <- median(train_wind$windspeed)
mean((wind_pre_median - val_wind$windspeed)^2)
```

**method3 . 평균(Mean)으로 대치 **  

```{r}
wind_pre_mean <- mean(train_wind$windspeed)
